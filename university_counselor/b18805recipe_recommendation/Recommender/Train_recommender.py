...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        number_of_training_data = number_of_training_data//10000
        print('change number_of_training_data=', number_of_training_data)
        print('len(testRatings)=', len(testRatings))

        # fast_rato = 1000
        # evens = list(range(0, len(testRatings), fast_rato))

        # train_dataset = torch.utils.data.Subset(train_dataset, evens)

        # testRatings = testRatings[:len(testRatings)//1000]
        ...
        for epoch in range(my_epochs):
            ...
            if epoch % args.verbose == 0:
                ...
                traing_history.append((hr, ndcg, loss))
                if hr > best_hr:
        ...
        r = shared["history"]
        print('Testing back thread is receving:', r)
        # --------- share data end -------
