...
def recommend():
    ...
    if request.method == "GET":
        ...
        for blog in blogs:
            if similarity(blog.title, user.school_class) > 0.9:
...
def activity_home(id):
    ...
    if request.method == "GET":
        # 到数据库查询ppt详情
        blog = Blog.query.filter_by(id=id).first_or_404()
        teacherWork = TeacherWork.query.filter_by(activity_id=id).first()
        print(id, blog, "in query_blog", "@" * 20)
        # 渲染ppt详情页面
        return rt("activity.html", blog=blog, teacherWork=teacherWork)
    else:
        return "", 204
